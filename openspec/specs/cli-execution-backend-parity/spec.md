...
系统 MUST 将 `backendMode / remoteBackendHost / remoteBackendToken` 作为 `CLI 验证` 面板内的 shared execution backend 配置展示，而不是继续作为 `Codex` tab 内的隐含专属配置。

#### Scenario: execution backend controls are shared above engine-specific tabs

- **WHEN** 用户进入 `CLI 验证` 面板
- **THEN** 系统 MUST 在 `Codex / Claude Code` tabs 之外展示 shared execution backend 区块
- **AND** 该区块 MUST 至少包含 `backendMode`
- **AND** 当 `backendMode = remote` 时，系统 MUST 展示 `remoteBackendHost` 与 `remoteBackendToken`
- **AND** 用户 MUST 不需要切到 `Codex` tab 才能发现这些 transport 配置

#### Scenario: claude tab inherits the same backend mode semantics

- **WHEN** 用户切换到 `Claude Code` tab
- **THEN** Claude 相关 doctor / execution 语义 MUST 继续遵循当前 shared execution backend 配置
- **AND** 系统 MUST NOT 让 UI 暗示 `backendMode` 只影响 Codex 而不影响 Claude

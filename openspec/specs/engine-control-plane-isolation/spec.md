# engine-control-plane-isolation Specification

## Purpose

Define cross-engine runtime boundaries so each engine control-plane protocol is sent only to a runtime that proves the required capability, and contaminated history payloads are contained without modifying user transcripts.
...
- **THEN** `openspec validate fix-claude-control-plane-session-contamination --strict` MUST pass

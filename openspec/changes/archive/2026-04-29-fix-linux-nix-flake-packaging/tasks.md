...
- [x] 3.3 Validate Nix npm fetcher settings. `[P0][依赖: 2.2, 3.2][输入: lockfile format, peer dependency behavior, Nix build output][输出: npmDepsHash/npmDepsFetcherVersion used for fixed-output dependency source; npmFlags applied only if required][验证: nix build dependency fetch succeeds; npmDepsHash and any --legacy-peer-deps use are documented as build-resolution behavior]`
...
- 2026-04-28 PR #428 follow-up commit `fe252675` switched the frontend dependency source to `importNpmLock`, but local lockfile audit found 872 `node_modules/*` package entries without `resolved`, including 53 root dependencies. This violates nixpkgs `importNpmLock`'s evaluation contract and causes `attribute 'resolved' missing`, so this change keeps `npmDepsHash` + `npmDepsFetcherVersion = 2` with the 0.4.11 hash above.

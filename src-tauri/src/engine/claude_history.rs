...
fn is_filtered_message(text: &str) -> bool {
    text.starts_with("<command-name>")
        ...
        || text.contains("Caveat: The messages below were generated by the user while running local commands")
}
...
async fn scan_session_file(path: &Path) -> Option<ClaudeSessionSummary> {
    ...
    let first_message = first_user_message.unwrap_or_else(|| format!("Session {}", &session_id[..8.min(session_id.len())]));
        ...
        if role == "assistant" {
            if let Some(usage) = msg.get("usage") {
                last_usage = Some(ClaudeSessionUsage {
                    ...
                    cache_creation_input_tokens: usage.get("cache_creation_input_tokens").and_then(|v| v.as_i64()),
                    cache_read_input_tokens: usage.get("cache_read_input_tokens").and_then(|v| v.as_i64()),
                });
            }
        }
                        ...
                        "tool_result" => {
                            ...
                            if !result_content.is_empty() {
                                ...
                                messages.push(ClaudeSessionMessage {
                                    ...
                                    timestamp: timestamp.clone(),
                                    kind: "tool".to_string(),
                                    tool_type: Some(if is_error { "error".to_string() } else { "result".to_string() }),
                                    title: Some(if is_error { "Error".to_string() } else { "Result".to_string() }),
                                });
                            }
                        }
                        _ => {}
